...
import { ArrowLeft, Settings, Check, Trash2, MoreHorizontal, Bell } from 'lucide-react'
...
const Notification = () => {
  ...
  const fetchNotifications = async () => {
    try {
      const { notifications: notifs } = await notificationsApi.getNotifications(50, 0)
    ...
    <div className="max-w-5xl mx-auto min-h-screen bg-gray-50">
            ...
            >
              ...
              {filter === 'Unread' && unreadCount > 0 && (
                <span className={`ml-1.5 px-1.5 py-0.5 text-xs rounded-full ${
                  activeFilter === filter ? 'bg-white text-blue-600' : 'bg-blue-100 text-blue-600'
                }`}>
                  {unreadCount}
                </span>
              )}
      ...
      {unreadCount > 0 && (
        <div className="p-3 bg-blue-50 md:hidden">
          <button onClick={markAllAsRead} className="w-full py-2 text-sm text-blue-600 font-medium">
            Mark all as read
          </button>
        </div>
      )}
          ...
          filteredNotifications.map((notification) => (
            ...
            >
              ...
              <div className="relative flex-shrink-0">
                <img
                  src={notification.actor?.profile_picture || '/img/avatar.png'}
                  alt="User avatar"
                  className="w-12 h-12 md:w-10 md:h-10 rounded-full object-cover"
                />
                {!notification.is_read && (
                  <div className="absolute -top-1 -right-1 w-3 h-3 bg-blue-600 rounded-full border-2 border-white md:hidden" />
                )}
              </div>
